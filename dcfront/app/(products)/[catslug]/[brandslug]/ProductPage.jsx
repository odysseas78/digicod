...
//   document.cookie = cnam + "=" + cvalue + ";" + expires + ";path=/";
// }
...
async function getProducts(simste, setProduct, sparams, params, region){
        simste.pset(["productloader"], true)
        const Data = await fetchClient('GET', 'product', { brand__slug: params.brandslug, regions__contains: (sparams.get('r'))})
        if(Data?.detail === "Invalid token Error: HTTP 401") window.location.reload()
        setProduct(Data.data)
        console.log(Data)
        Data && simste.pset(["product"], Data.data) 
        setTimeout(()=>{simste.pset(["productloader"], false)},0)
}

// async function postProducts(simste, setProduct, sparams, params, region){
//         simste.pset(["productloader"], true)
//         const Data = await fetchClient('POST', 'product', { brand__slug: params.brandslug, regions__contains: (sparams.get('r'))})
//         if(Data?.detail === "Invalid token Error: HTTP 401") window.location.reload()
//         setProduct(Data.data)
//         console.log(Data)
//         Data && simste.pset(["product"], Data.data) 
...
// const dateref = useRef(Date.now())
 useEffect(() => {
  setIsClient(true)
  // console.log(isFirstRender2.current, sparams.get('r')) 
  const id = setTimeout(() => {
          if(isFirstRender2.current !== sparams.get('r')){
            // setLoading(false)
                //  isFirstRender2.current && console.log("WWWWWWW: ", sparams.get('r')) 
                 isFirstRender2.current && getProducts(simste, setProduct, sparams, params, sparams.get('r'))
              isFirstRender2.current = sparams.get('r')
              // setLoading(false)
          } 
          //  console.log(Date.now() - dateref.current)
        }, 50)
...
useEffect(() => {
   setLoading('productloader')
  const id = setTimeout(async () => {
    console.log("DDDDDDDDDDDDDDDDDDDDD: ", sparams.get('r')) 
     await getProducts(simste, setProduct, sparams, params, sparams.get('r'))
     setLoading(false)
  }, 100)
  return () => {clearTimeout(id)} 
}, [])
...
// const mounted = useMounted()
  ...
  if(!IsClient || ((productloader || simste.pget(["basketloader"])) && loading === "productloader")) return <Loading />
...
{/* <Button onClick={() => {po.pr_price = po.pr_price + 1}} >lkölköl</Button> */}
        <div className='flex flex-row justify-between items-start w-full' >
          <div className={'flex flex-col flex-wrap justify-center w-full p-px gap-2'}>
            <div className='text-[clamp(1rem,1.2vw,1.3rem)]!' >
            ...
            <div className='flex flex-row gap-5 relative'>
              ...
              <div className='flex flex-col gap-[0.30rem] sm:gap-[0.65rem] items-start text-[clamp(0.65rem,1vw,0.75rem)]!' >
                <div className='mt[-18px] notranslate'>
                  <SelectRegion regions={regions} />
                </div>
                <div className='flex flex-row gap-1 items-center'>
                  <Mail className='text-green-600' size={20} /> <div className=''>Instant delivery</div>
                </div>
                {/* ####### dcoin cashback ########### */}
            ...
            <div className="flex flex-row flex-wrap justify-center gap-4 w-full mt-4">
              {
                product.map((p) => {
                  ...
                  return (
                  <div key={p.slug} className='flex flex-row gap-[7px] items-cente notranslate text-[clamp(0.7rem,1vw,0.85rem)]!' >
                     <PriceLabelDB key={p.slug} {...prp} postBasket={postBasket} />
                  </div>
                )  
                })
              }
          ...
          <div className="flex flex-row flex-wrap self-start justify-center items-start mt-4 gap-4 w-full relative">
            {
              product.map((p) => {
                ...
                return (
                  <div key={p.id} className='flex flex-row gap-[7px] items-center notranslate text-[clamp(0.7rem,1vw,0.85rem)]!' >
                     <PriceLabelDB {...prp} postBasket={postBasket} />
                  </div>
                ) 
              })
            }
