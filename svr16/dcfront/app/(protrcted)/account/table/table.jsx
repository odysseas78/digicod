               ...
               {/* <Collapse trigger={collapse} p={p} > */}
                  <div className="m-[2px] p-[1px] max-w-full bg-background rounded-b-sm! shadow shadow-neutral-400/70 dark:shadow-neutral-500/70">
                     <div className='w-full flex flex-row justify-start gap-3 p-3 border-b' >
                        ...
                        {<Button disabled={p.status === 'processing'} size='xs' variant='secondary' className='hover:ring-[0.5px] active:scale-95 rounded-sm' >
                           Delete
                        </Button>}
                        ...
                        Object.keys(p.cart.basket_products).map((k)=>{
                           ...
                           return (
                              <TableRow key={k} className='border-y!' >
                                    <TableCell className="text-left"> {basketpr.title}</TableCell>
                                    <TableCell className="text-right"> {basketpr.qty}</TableCell>
                                    <TableCell className="text-right"> {(basketpr.price * p.currency.price).toFixed(2)} {p.cart.currency.sign}</TableCell>
