...
def defaultpay_send(type, orderqs, id, amount, currency, urlOk, urlKo, email, name, ip):
    
    if type == 'o':
        paymeth = orderqs.cart.payment_method.name
        items = []
        ...
        paymeth = orderqs.payment_method.name
        items = [{'id':str(id),'name':'Digipoints top-up', 'price': str(amount)}]
    
    pmid = get_paymethod(paymeth)
    
    result = create_invoice(paymentMethod=pmid.get('identifier'), amount=str(amount), currency=currency, items=items, orderid=str(id), 
